...
class MultiCamPhotometricLoss(LossBase):
    """
    ...
    def __init__(self, num_scales=4, ssim_loss_weight=0.85, occ_reg_weight=0.1, smooth_loss_weight=0.1,
                 ...
                 progressive_scaling=0.0, padding_mode='zeros', automask_loss=False):
        super().__init__()
        self.n = num_scales
    ...
    def warp_ref_image(self, inv_depths, ref_image, K, ref_K, pose, cam_to_car):
        """
        ...
        rot_cam_to_car = torch.eye(4, device=pose.mat.device, dtype=pose.mat.dtype).repeat([len(pose), 1, 1])
        rot_cam_to_car[:, :3, :3] = cam_to_car[:, :3, :3]

        rot_car_to_cam = torch.eye(4, device=pose.mat.device, dtype=pose.mat.dtype).repeat([len(pose), 1, 1])
        rot_car_to_cam[:, :3, :3] = torch.transpose(rot_car_to_cam[:, :3, :3], -2, -1)

        car_to_cam = rot_car_to_cam.clone()
        car_to_cam[:, :3, -1] = torch.bmm(-1.*rot_car_to_cam[:, :3, :3], cam_to_car[:, :3, -1].unsqueeze(-1)).squeeze(-1)

        pose = Pose(car_to_cam) @ Pose(rot_cam_to_car) @ pose @ Pose(cam_to_car)
    ...
    def forward(self, image, context, inv_depths, K, extrinsics, poses, progress=0.0):
        """
        ...
        for j, (ref_image, pose) in enumerate(zip(context, poses)):
            ...
            ref_warped = self.warp_ref_image(inv_depths, ref_image, K, K, pose, extrinsics)
