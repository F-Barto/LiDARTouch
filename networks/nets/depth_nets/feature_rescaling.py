...
class FeatRescaling(NetworkBase):
    """
    ...
    def __init__(self, image_extractor_name, image_extractor_hparams, lidar_extractor_name, lidar_extractor_hparams,
        ...
        # at each resblock rescale
        self.params_gens = nn.ModuleDict()
        for i in range(len(self.num_ch_enc)):
            # one factor by channel
            self.params_gens[f"gammas_{i}"] = nn.Linear(self.lidar_ch_enc, self.num_ch_enc[i])
            self.params_gens[f"betas_{i}"] = nn.Linear(self.lidar_ch_enc, self.num_ch_enc[i])
    ...
    def forward(self, image_input, pc_input):
        ...
        for i in range(len(self.num_ch_enc)):
            ...
            rescaled_feature = torch.mul(cam_features[i], gammas_expanded) + betas_expanded

            self.rescaled_features.append(rescaled_feature)

        outputs = self.decoder(self.rescaled_features)
