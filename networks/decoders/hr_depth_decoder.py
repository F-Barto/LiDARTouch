...
class HRDepthDecoder(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), mobile_encoder=False):
        ...
        for j in range(5):
            for i in range(5 - j):
                ...
                self.convs["X_{}{}_Conv_0".format(i, j)] = PaddedConv3x3Block(num_ch_in, num_ch_out)

                # X_04 upconv 1, only add X_04 convolution
                if i == 0 and j == 4:
                    num_ch_in = num_ch_out
                    num_ch_out = self.num_ch_dec[i]
                    self.convs["X_{}{}_Conv_1".format(i, j)] = PaddedConv3x3Block(num_ch_in, num_ch_out)
        ...
        for index in self.non_attention_position:
            ...
            if mobile_encoder:
                ...
                if col == 1:
                    self.convs["X_{}{}_Conv_1".format(row + 1, col - 1)] = PaddedConv3x3Block(num_ch_enc[row + 1] // 2 +
                                                                                     self.num_ch_enc[row],
    ...
    def forward(self, input_features):
        ...
        return [x, features["X_04"], features["X_13"], features["X_22"]]